...
class TraditionalAgent:
    ...
    def _minimax(self, board, alpha, beta, depthleft):
        """
        Searching the best move using minimax algorithm with negamax implementation and
        alphabeta cutoff enhancement to reduce computation time
    ...
    @staticmethod
    def make_move(board:chess.Board, move: chess.Move) -> chess.Board:
        new_board = board.copy()
        new_board.push(move)
        return new_board

    def selectmove(self, board: chess.Board = None, depth: int = None):
        ...
        for move in board.legal_moves:
            board.push(move)
            boardValue = -self._minimax(board, -beta, -alpha, depth - 1)
            ...
            if boardValue > alpha:
                alpha = boardValue
            board.pop()
        # bestMove = bestMove if white_turn else self._inverse_move(bestMove)
...
if __name__ == "__main__":
    ...
    start = datetime.now()
    agent = TraditionalAgent(max_depth=3)
    chosen_move = agent.selectmove(board)
    print("execute time = ", (datetime.now() - start))
    print(chosen_move.uci())

    start = datetime.now()
    agent = TraditionalAgent(max_depth=3)
    chosen_move_black = agent.selectmove(board_black)
    print("execute time = ", (datetime.now() - start))
    print(chosen_move_black.uci())
